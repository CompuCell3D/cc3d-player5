# Courtesy of https://github.com/skywind3000/terminal/blob/master/terminal.py
# MIT License
...
class configure(object):
    ...
    def win32_path_short(self, path):
        ...
        if retval <= 0:
            import ctypes
            print
            'ERROR(%d): %s' % (ctypes.GetLastError(), path)
            return ''
        return shortpath
...
if __name__ == '__main__':
    def test1():
        cfg = configure()
        cfg.darwin_open_terminal('111', ['ls -la /', 'read -n1 -rsp press\\ any\\ key\\ to\\ continue\\ ...'])
